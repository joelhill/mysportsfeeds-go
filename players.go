...
func (s *Service) Players(options *PlayersOptions) (PlayersIO, int, error) {
	...
	if response.StatusCode < 200 || response.StatusCode > 300 {
		s.Logger.Errorf("client: something went wrong making the get request for Players: %s", err.Error())
		return mapping, response.StatusCode, err
	}

	s.Logger.Infof("Players Status Code: %d", response.StatusCode)

	if jErr := json.NewDecoder(response.Body).Decode(&mapping); jErr != nil {
		s.Logger.Errorf("client: error decoding response for Players: %s", err.Error())
		return mapping, response.StatusCode, err
	}
